...
class YOLOv11Detector:
    ...
    # YOLOv11 model classes (from training)
    # 0: Drinking, 1: Eating, 2: Gloves, 3: Googles, 4: Head Mask,
    # 5: Lab Coat, 6: Mask, 7: No Gloves, 8: No googles, 9: No Head Mask,
    # 10: No Lab coat, 11: No Mask
    CLASS_NAMES = {
        0: "Drinking",
        1: "Eating",
        2: "Gloves",
    ...
    VIOLATION_CLASS_MAPPING = {
        ...
        8: "safety goggles",  # No googles -> missing safety goggles
        9: "head mask",  # No Head Mask -> missing head mask
        10: "lab coat",  # No Lab coat -> missing lab coat
        11: "face mask",  # No Mask -> missing face mask
    }
    ...
    def __init__(self):
        ...
        self.violation_threshold = getattr(settings, "VIOLATION_CONFIDENCE_THRESHOLD", 0.3)
    ...
    def initialize(self):
        ...
        if model_path.suffix == ".onnx":
            ...
            logger.warning("⚠️ Consider using PyTorch (.pt) model for better compatibility")
        logger.info("=" * 80)
        ...
        if not model_path.exists():
            ...
            logger.error(f"Parent directory contents: {list(model_path.parent.glob('*')) if model_path.parent.exists() else 'N/A'}")
            logger.warning("Falling back to mock detector for development")
            self._initialized = True
            return
        ...
        try:
            ...
            try:
                ...
                logger.info(f"✓ YOLOv11 model verification test passed - model is ready")
    ...
    def detect(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        if self.model is None:
            ...
            logger.warning("YOLOv11 model is None - using mock detector. Check model loading.")
        ...
        try:
            ...
            if action_violations:
                logger.info(f"✓ Action violations: {[a.get('action') for a in action_violations]}")
            
            if len(detections) == 0:
                logger.warning("⚠️ YOLOv11 returned 0 detections - model may not be working correctly")
    ...
    def _detect_pytorch(self, frame: np.ndarray) -> List[Dict]:
        ...
        # We'll filter by class-specific thresholds in _parse_detections
    ...
    def _detect_onnx(self, frame: np.ndarray) -> List[Dict]:
        ...
        output = outputs[0]  # Shape: [batch, num_detections, features] or [batch, num_anchors, features]
        
        # Remove batch dimension if present
        if len(output.shape) == 3:
            output = output[0]  # [num_detections, features] or [num_anchors, features]
        
        logger.info(f"🔍 Processing ONNX output: shape={output.shape}, first detection sample={output[0] if len(output) > 0 else 'empty'}")
        ...
        for detection in output:
            ...
            x1, y1, x2, y2 = float(detection[0]), float(detection[1]), float(detection[2]), float(detection[3])
    ...
    def _mock_detect(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "violation_detections": {
                "safety goggles": [
                    {"box": [150, 100, 250, 180], "score": 0.75, "mask": None, "class_name": "no safety goggles"}
                ],
            },
            "action_violations": [],
            "frame_shape": (h, w),
